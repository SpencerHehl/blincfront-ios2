...
export class AuthService {
    ...
    newUser(passedUser){
        ...
        return this.http.post('http://104.238.138.146:80/user/newuser', newUser, options).map((response: Response) => {
    ...
    login(){
        ...
        console.log(options);
        console.log(body);
        return this.http.post('http://104.238.138.146:80/user/login', body, options)
    ...
    logout(){
        ...
        let options = new RequestOptions({headers: headers});
        return this.http.post('http://104.238.138.146:80/user/logout', body, options)
        .map((response) =>{
            return response.json();
        })
        .catch(this.handleError);
